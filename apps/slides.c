...
/*
 * Glyph/codepoint mapping extracted from fonts/lat1u-08.psf so clipboard
 * conversions never emit glyphs that the bundled font cannot render.
 */
static const uint16_t g_psf_glyph_to_cp[256] = {
    65533, 8776, 338, 339, 9670, 9225, 9228, 9229,
    9226, 9617, 9618, 9619, 9608, 9604, 9600, 9612,
    9616, 9252, 9227, 8804, 8805, 8800, 9664, 9654,
    8593, 8595, 8594, 8592, 8597, 8596, 8629, 960,
    ...
    120, 121, 122, 123, 124, 125, 126, 376,
    192, 193, 194, 195, 196, 197, 198, 199,
    200, 201, 202, 203, 204, 205, 206, 207,
    208, 209, 210, 211, 212, 213, 214, 215,
    216, 217, 218, 219, 220, 221, 222, 223,
    9251, 161, 162, 163, 164, 165, 166, 167,
    168, 169, 170, 171, 172, 173, 174, 175,
    176, 177, 178, 179, 180, 181, 182, 183,
    184, 185, 186, 187, 188, 189, 190, 191,
    63489, 9589, 9590, 9492, 9591, 9474, 9484, 9500,
    9588, 9496, 9472, 9524, 9488, 9508, 9516, 9532,
    63491, 9593, 9594, 9495, 9595, 9475, 9487, 9507,
    9592, 9499, 9473, 9531, 9491, 9571, 9523, 9547,
...
static const struct { uint16_t cp; unsigned char glyph; } g_psf_cp_to_glyph[] = {
    {32, 32},
    {33, 33},
    {34, 34},
    {35, 35},
...
static const size_t g_psf_cp_to_glyph_len = sizeof(g_psf_cp_to_glyph) / sizeof(g_psf_cp_to_glyph[0]);
...
static unsigned char glyphFromCodepoint(int cp) {
    for (size_t i = 0; i < g_psf_cp_to_glyph_len; i++) {
        if (g_psf_cp_to_glyph[i].cp == (uint16_t)cp)
            return g_psf_cp_to_glyph[i].glyph;
    }
    return '?';
}

static int codepointFromGlyph(unsigned char glyph) {
    return (int)g_psf_glyph_to_cp[glyph];
}
