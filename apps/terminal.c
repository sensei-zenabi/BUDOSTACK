...
static void terminal_buffer_set_cursor_addressed(struct terminal_buffer *buffer, size_t column, size_t row) {
    ...
    size_t target_column = column;
    if (buffer->origin_mode) {
        size_t top = 0u;
        size_t bottom = 0u;
        terminal_buffer_resolve_scroll_region(buffer, &top, &bottom);
        target_row += top;
    ...
    terminal_buffer_set_cursor(buffer, target_column, target_row);
}
...
static void ansi_apply_csi(struct ansi_parser *parser, struct terminal_buffer *buffer, unsigned char command) {
    ...
    case 'f': { /* Cursor Position */
        ...
        terminal_buffer_set_cursor_addressed(buffer, (size_t)(column - 1), (size_t)(row - 1));
        break;
    }
    case 'L': { /* Insert Line */
    ...
    case 'd': { /* Vertical Position Absolute */
        ...
        terminal_buffer_set_cursor_addressed(buffer, buffer->cursor_column, (size_t)(row - 1));
        break;
    }
    case 'r': { /* Set scrolling region */
        ...
        terminal_buffer_set_cursor_addressed(buffer, 0u, 0u);
        break;
    }
    case 'h':
    case 'l':
            ...
            for (size_t i = 0u; i < parser->param_count; i++) {
                ...
                case 7: /* autowrap */
                    if (command == 'h') {
                        ...
                        buffer->autowrap_enabled = 0;
                        if (buffer->columns > 0u && buffer->cursor_column >= buffer->columns) {
