...
# Find all .c files in the budo folder (developer-facing libraries)
BUDO_SRCS = $(shell find ./budo -type f -name '*.c')
BUDO_OBJS = $(BUDO_SRCS:.c=.o)
...
# Exclude command, app, and library sources from the main executable sources.
NON_COMMAND_SOURCES = $(filter-out ./commands/% ./apps/% ./games/% ./lib/% ./budo/% ./utilities/%, $(ALL_SOURCES))
...
$(TARGET): $(NON_COMMAND_OBJECTS) $(LIB_OBJS) $(BUDO_OBJS)
	@echo "Linking $(TARGET)..."
	$(CC) $(NON_COMMAND_OBJECTS) $(LIB_OBJS) $(BUDO_OBJS) $(LDFLAGS) -o $(TARGET)

# For each executable, link its corresponding object file with the lib objects.
$(COMMANDS_EXES) $(APPS_EXES) $(GAMES_EXES) $(UTILITIES_EXES): %: %.o $(LIB_OBJS) $(BUDO_OBJS)
	@echo "Linking $@..."
	$(CC) $< $(LIB_OBJS) $(BUDO_OBJS) $(LDFLAGS) -o $@
